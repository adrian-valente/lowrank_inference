...
from low_rank_rnns.modules import SupportLowRankRNN, LowRankRNN, FullRankRNN
from low_rank_rnns.helpers import gram_factorization
from low_rank_rnns.plotting import center_axes
import torch
...
def inactivate_pop(net, pop):
    size = net.hidden_size - np.sum(pop)
    wi = net.wi_full[:, ~pop]
    wo = net.wo_full[~pop]

    if isinstance(net, LowRankRNN):
        m = net.m[~pop]
        n = net.n[~pop]
        net2 = LowRankRNN(net.input_size, size, net.output_size, net.noise_std, net.alpha, net.rank,
                          wi_init=wi, m_init=m, n_init=n, wo_init=wo)
    elif isinstance(net, FullRankRNN):
        mask = np.ix_(~pop, ~pop)
        wrec = net.wrec[mask]
        net2 = FullRankRNN(net.input_size, size, net.output_size, net.noise_std, net.alpha, net.rho,
                           wi_init=wi, wrec_init=wrec, wo_init=wo)
    else:
...
def pop_scatter_linreg(vec1, vec2, pops, n_pops=None, colors=('blue', 'red', 'green', 'gray'),
                       linreg=True, figsize=(5, 5), ax=None, dotsize=4, ls=None, lw=None):
    """
