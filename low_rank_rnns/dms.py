...
from low_rank_rnns.plotting import remove_axes
import torch
...
def generate_ordered_inputs(repeats=1, std=std_default, stim1_dur=min_stimulus1_duration_discrete,
                            delay_dur=min_delay_duration_discrete,
                            stim2_dur=min_stimulus2_duration_discrete):
    duration = fixation_duration_discrete + stim1_dur + delay_dur + stim2_dur + decision_duration_discrete
    stim1_beg = fixation_duration_discrete
    stim1_end = fixation_duration_discrete + stim1_dur
    stim2_beg = stim1_end + delay_dur
    stim2_end = stim2_beg + stim2_dur

    x = std * torch.randn(4 * repeats, duration, 2)
    y = torch.zeros(4 * repeats, duration, 1)
    mask = torch.zeros(4 * repeats, duration, 1)

    # Trials A-A
    x[:repeats, stim1_beg:stim1_end, 0] += 1
    x[:repeats, stim2_beg:stim2_end, 0] += 1
    y[:repeats, stim2_end:] = 1
    # Trials A-B
    x[repeats:2*repeats, stim1_beg:stim1_end, 0] += 1
    x[repeats:2*repeats, stim2_beg:stim2_end, 1] += 1
    # Trials B-A
    x[2*repeats:3*repeats, stim1_beg:stim1_end, 1] += 1
    x[2*repeats:3*repeats, stim2_beg:stim2_end, 0] += 1
    # Trials B-B
    x[3*repeats:, stim1_beg:stim1_end, 1] += 1
    x[3*repeats:, stim2_beg:stim2_end, 1] += 1
    y[3*repeats:, stim2_end:] = 1

    mask[:, stim2_end:] = 1
    return x, y, mask, [stim1_beg, stim1_end, stim2_beg, stim2_end, duration]
